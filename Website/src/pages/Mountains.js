...
function Mountains() {
  ...
  const openDeleteModal = (mountain) => {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      await apiService.deleteMountain(mountainToDelete.id);
      setMountains(prev => prev.filter(mountain => mountain.id !== mountainToDelete.id));
      setFeedback('Mountain deleted successfully.');
      setFeedbackType('success');
    } catch (err) {
      console.error('Error deleting mountain:', err);
      setFeedback('Failed to delete mountain. Please try again.');
                    ...
                    <td className="px-6 py-4">
                      <div className="flex gap-2">
                        ...
                        <button 
                          ...
                          onClick={() => openDeleteModal(mountain)}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-6">
            <div className="flex items-start justify-between mb-4">
              <div>
                <h3 className="text-lg font-semibold text-gray-900">Delete mountain?</h3>
                <p className="mt-1 text-sm text-gray-600">
                  This action cannot be undone. This will permanently remove{' '}
                  <span className="font-semibold">{mountainToDelete.name}</span> from your list.
                </p>
              </div>
              <button
            ...
            <div className="mb-6 rounded-lg bg-gray-50 border border-gray-200 p-4 text-sm text-gray-700">
              <p className="font-semibold text-gray-900 mb-1">
                {mountainToDelete.name}
              </p>
              <p className="flex items-center gap-1 text-gray-600">
                <span>📍</span>
                <span>{mountainToDelete.location}</span>
              </p>
              <p className="mt-1 text-gray-600">
                Difficulty: <span className="font-medium">{mountainToDelete.difficulty}</span>
              </p>
            </div>
            ...
            <div className="flex justify-end space-x-3">
              ...
              >
                {deleteLoading ? 'Deleting...' : 'Yes, Delete Mountain'}
              </button>
            </div>
          </div>
        </div>
