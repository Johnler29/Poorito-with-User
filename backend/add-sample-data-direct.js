...
async function addSampleData() {
  try {
    console.log('Adding sample mountain details data...');

    // Get the first mountain ID
    ...
    console.log('Using mountain:', mountains[0].name, 'ID:', mountainId);

    // Sample data for Mount Pinatubo
    const sampleData = [
      // What to Bring
      {
    ...
    for (const item of sampleData) {
      try {
        ...
        if (error) {
          console.error('Error inserting item:', item.item_name, error.message);
        } else {
          successCount++;
          console.log('✓ Inserted:', item.item_name);
        }
      } catch (err) {
        console.error('Exception inserting item:', item.item_name, err.message);
      }
    }

    console.log(`\nSuccessfully inserted ${successCount} out of ${sampleData.length} items`);
